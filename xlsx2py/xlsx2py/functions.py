...
def funcGift(mapDict, dctData, chilidDict, data):
    p = 0
    l = []
    # print(data)
    if data == None or data == "0.0":
        return l
    for pair in data.split(";"):
        if pair == "":
            continue
        underLineIndex = pair.find("-")
        colonIndex = pair.find(":")

        id = pair[0:underLineIndex]
        amount = int(pair[underLineIndex + 1:colonIndex])
        p = p + int(pair[colonIndex + 1:len(pair)])

        p2 = dict(id=id, amount=amount, percent=p)

        l.append(p2)
    return l


def funcSelectGift(mapDict, dctData, chilidDict, data):
    dic = {}
    if data == None or data == "0.0":
        return dic
    for pair in data.split(";"):
        ...
        id = pair[0:underLineIndex]
        amount = int(pair[underLineIndex + 1:len(pair)])
