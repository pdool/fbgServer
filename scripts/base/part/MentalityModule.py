...
class MentalityModule:
    ...
    def UpDateMentalityInfo(self, cardId):
        for item in self.addPropertyMentality:
            self.SetObjectValue(cardId, item["name"], item["number"] + self.GetObjectValue(cardId, item["name"]))
        ...
        for i in range(len(propertyList["values"])):
            ...
            for item in self.addPropertyMentality:
                if str(index) in item["name"]:
                    self.SetObjectValue(cardId, item["name"][1:],
                                        item["number"] + self.GetObjectValue(cardId, item["name"][1:]))
    ...
    def ComputeAdd(self, cardId, property, material):
        ...
        objectValue = self.GetObjectValue(cardId, property)
        if material == 0:
    ...
    def GetObjectValue(self, cardId, property):
        ...
        card = KBEngine.entities.get(cardId)
        # TODO: 不需要强转
        return int(getattr(card, property))
